    ...
    def __init__(self, config):
        ...
        if self.use_one_logger and dist.get_rank() == 0 and not self.disable_wandb:
            if OneLoggerUtils is None:
                raise ImportError(
                    "One Logger is not installed. Run training with --no-one-logger "
                    "or install one_logger_utils."
                )
    ...
    def train(self):
        ...
        try:
            while True:
                ...
                if (not self.config.no_visualize) and self.vis_interval > 0 and (self.step % self.vis_interval == 0):
                    if self.one_logger is not None:

...
def _read_prompt_lines(path):
    with open(path, encoding="utf-8") as f:
        prompts = [line.strip() for line in f]
    empty_lines = [index + 1 for index, prompt in enumerate(prompts) if not prompt]
    if empty_lines:
        preview = ", ".join(str(line) for line in empty_lines[:5])
        raise ValueError(f"Blank prompts found in {path} at line(s): {preview}")
    return prompts
...
class TextDataset(Dataset):
    def __init__(self, prompt_path, extended_prompt_path=None):
        self.prompt_list = _read_prompt_lines(prompt_path)

        if extended_prompt_path is not None:
            self.extended_prompt_list = _read_prompt_lines(extended_prompt_path)
...
class TwoTextDataset(Dataset):
    ...
    def __init__(self, prompt_path: str, switch_prompt_path: str):
        self.prompt_list = _read_prompt_lines(prompt_path)
        self.switch_prompt_list = _read_prompt_lines(switch_prompt_path)

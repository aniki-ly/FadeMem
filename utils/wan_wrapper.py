...
from wan.modules.causal_model_fademem import CausalWanModel
...
class WanVAEWrapper(torch.nn.Module):
    ...
    def decode_to_pixel_stream_chunk(self, latent: torch.Tensor, chunk_size: int = 120) -> torch.Tensor:
        """Decode long videos in chunks while preserving causal VAE state."""
        zs = latent.permute(0, 2, 1, 3, 4)
        ...
        for u in zs:
            decoded_chunks = []
            self.model.clear_cache()
            for start_idx in range(0, u.shape[1], chunk_size):
                end_idx = min(start_idx + chunk_size, u.shape[1])
                chunk = u[:, start_idx:end_idx, :, :]
                decoded_chunk = self.model.cached_decode(
                    chunk.unsqueeze(0), scale).float().clamp_(-1, 1).squeeze(0)
                decoded_chunks.append(decoded_chunk.cpu())
                del decoded_chunk
                torch.cuda.empty_cache()
            decoded = torch.cat(decoded_chunks, dim=1)
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        if is_causal:
            if use_infinite_attention:
                raise ValueError("use_infinite_attention is not included in the FadeMem release")
            self.model = CausalWanModel.from_pretrained(
                f"wan_models/{model_name}/",
                local_attn_size=local_attn_size,
                sink_size=sink_size,
                fademem_enabled=fademem_enabled,
                fademem_summary_slots=fademem_summary_slots,
                fademem_warp_type=fademem_warp_type,
                fademem_warp_beta=fademem_warp_beta,
                fademem_span_gamma=fademem_span_gamma,
            )
        else:
    ...
    def adding_cls_branch(self, atten_dim=1536, num_class=4, time_embed_dim=0) -> None:
        # This classification branch is specific to Wan2.1-T2V-1.3B.

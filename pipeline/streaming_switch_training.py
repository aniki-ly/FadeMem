...
class StreamingSwitchTrainingPipeline(StreamingTrainingPipeline):
    ...
    def _recache_after_switch(self, output, current_start_frame, new_conditional_dict, local_start_frame=None, switch_recache_frames=None):
        recache_window = self.local_attn_size if self.local_attn_size > 0 else 21

        if not self.global_sink:
            # Reset KV cache content; keep end indices for recache bookkeeping.
            # Preserve fademem: far-history summaries remain useful across prompt switches.
        ...
        if switch_recache_frames is not None:
            frames_to_recache = torch.cat([switch_recache_frames, output], dim=1)[:, -recache_window:, ...]
            ...
            if local_start_frame is not None:
                num_recache_frames = min(local_start_frame, recache_window)
                frames_to_recache = output[:, -num_recache_frames:]
            else:
                num_recache_frames = min(current_start_frame, recache_window)
                frames_to_recache = output[:, -num_recache_frames:]
        ...
        block_mask = self.generator.model._prepare_blockwise_causal_attn_mask(
            ...
            local_attn_size=recache_window
        )

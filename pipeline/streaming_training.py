...
from wan.modules.causal_model_fademem import clear_checkpoint_forward_state


class _CacheDict:
    """Opaque dict wrapper invisible to ``pytree.tree_map``.

    FSDP's pre-forward hook uses ``pytree.tree_map`` to process forward inputs.
    Standard ``dict`` objects are registered pytree nodes and get shallow-copied
    on every forward call – new dict containers, same tensor leaf references.
    Any key added *after* initialization (e.g. ``fademem_k`` via ``ensure_cache``)
    is added only to the copy and silently lost when the copy is GC'd.

    Wrapping each cache dict in this class (not a registered pytree node) makes
    ``tree_map`` treat the whole object as a leaf, preserving identity across
    all nested FSDP forward passes.
    """
    ...
    # ---- dict-like API used by the rest of the codebase ----
...
class StreamingTrainingPipeline:
    def __init__(self,
        ...
        self.fademem_enabled = kwargs.get("fademem_enabled", False)

        slice_last_frames: int = int(kwargs.get("slice_last_frames", 21))
        if self.fademem_enabled:
            # Match inference: physical cache = attention window so that
            # eviction (and therefore fademem updates) happen at the same
            # frame offset during training and inference.
            kv_frames = self.local_attn_size
        else:
            kv_frames = self.local_attn_size + slice_last_frames
        self.kv_cache_size = kv_frames * self.frame_seq_length
        if DEBUG:
            print(f"[KV policy] local_attn_size={self.local_attn_size} slice_last_frames={slice_last_frames} fademem={self.fademem_enabled} -> kv_frames={kv_frames}")
    ...
    def _initialize_crossattn_cache(self, batch_size, dtype, device):
        ...
        for _ in range(self.num_transformer_blocks):
            crossattn_cache.append(_CacheDict({
    ...
    def clear_kv_cache(self):
        ...
        """
        clear_checkpoint_forward_state()
        ...
        if getattr(self, "kv_cache1", None) is not None:
            for blk in self.kv_cache1:
                ...
                if "fademem_k" in blk:
                    blk["fademem_k"].zero_()
                    blk["fademem_v"].zero_()
                    blk["fademem_center_frame"].fill_(-1)
                    if "fademem_kv_frame" in blk:
                        blk["fademem_kv_frame"].fill_(-1)
                    blk["fademem_span"].zero_()
                    blk["fademem_count"].zero_()

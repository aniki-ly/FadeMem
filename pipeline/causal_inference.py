...
class CausalInferencePipeline(torch.nn.Module):
    ...
    def inference(
        ...
        # Chunked VAE decode for long videos to avoid OOM; opt-in via top-level
        # config key vae_chunk_size (>0). Falls back to full-sequence decode.
        vae_chunk_size = int(getattr(self.args, "vae_chunk_size", 0) or 0)
        vae_stream_chunk_size = int(getattr(self.args, "vae_stream_chunk_size", 0) or 0)
        ...
        elif vae_stream_chunk_size > 0:
            video = self.vae.decode_to_pixel_stream_chunk(
                output.to(noise.device), chunk_size=vae_stream_chunk_size)
        elif vae_chunk_size > 0:
            video = self.vae.decode_to_pixel_chunk(
                output.to(noise.device), use_cache=False, chunk_size=vae_chunk_size)
        else:
